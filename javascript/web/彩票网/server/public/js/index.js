...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Lottery = function (_mix) {
	    ...
	    function Lottery() {
	        ...
	        _this.open_code_list_el = '#open_code_list';
	    ...
	    _createClass(Lottery, [{
	        ...
	        value: function updateState() {
	            ...
	            this.getState().then(function (res) {
	                _this2.issue = res.issue;
	                _this2.end_time = res.end_time;
	                _this2.state = res.state;
	                (0, _jquery2.default)(_this2.issue_el).text(res.issue);
	                (0, _jquery2.default)(_this2.state_el).text(res.stateText);
	                if (res.state === 0) {
	                    (0, _jquery2.default)(_this2.state_el).removeClass('bg-ccc').addClass('label-important');
	                } else if (res.state === 100) {
	                    (0, _jquery2.default)(_this2.state_el).removeClass('label-important').addClass('bg-ccc');
	                }
	                _this2.countdown(res.end_time, function (time) {
	                    (0, _jquery2.default)(_this2.countdown_el).html(time);
	                }, function () {
	                    setTimeout(function () {
	                        _this2.updateState();
	                        _this2.getOmit(_this2.issue).then(function (res) {
	                            (0, _jquery2.default)(_this2.omit_el).each(function (i, item) {
	                                (0, _jquery2.default)(item).text(_this2.omit.get(i));
	                            });
	                        });
	                        _this2.getOpenCode(_this2.issue).then(function (res) {});
	                    }, 500);
	                });
	            });
	        }
	        ...
	        value: function initEvent() {
	            // 玩法任二 ~ 任八 选项卡
	            (0, _jquery2.default)('#plays').on('click', 'li', this.changePlayNav.bind(this));
	
	            // 手动选择号码
	            (0, _jquery2.default)('.boll-list').on('click', '.btn-boll', this.toggleCodeActive.bind(this));
	            // 添加号码
	            (0, _jquery2.default)('#confirm_sel_code').on('click', this.addCode.bind(this));
	            // 操作区快捷选号
	            (0, _jquery2.default)('.dxjo').on('click', 'li', this.assistHandle.bind(this));
	
	            // 机选
	            (0, _jquery2.default)('.qkmethod').on('click', '.btn-middle', this.getRandomCode.bind(this));
	        }
	    }]);
...
/***/ (function(module, exports, __webpack_require__) {
	...
	var Base = function () {
	    ...
	    _createClass(Base, [{
	        ...
	        key: 'updateOpenCode',
	        value: function updateOpenCode(code) {
	
	            (0, _jquery2.default)(this.open_code_list_el).children('.ico-ball3').each(function (index, item) {
	                (0, _jquery2.default)(item).text(code[index]); // 更新开奖号码
	            });
	        }
	        ...
	        value: function toggleCodeActive(e) {
	            var $cur = (0, _jquery2.default)(e.currentTarget); // 触发事件监听器的对象元素
	        ...
	        value: function changePlayNav(e) {
	            var $cur = (0, _jquery2.default)(e.currentTarget); // 返回其事件监听器触发该事件的元素
	            $cur.addClass('active').siblings().removeClass('active');
	            // 设置当前玩法
	            this.cur_play = $cur.attr('desc').toLocaleLowerCase();
	            // 更新玩法提示语
	            (0, _jquery2.default)('#zx_sm span').html(this.play_list.get(this.cur_play).tip);
	            // 清除所有号码选中状态
	        ...
	        value: function addCode() {
	            ...
	            // 计算注数
	            var count = this.computeCount(active, this.cur_play);
	            if (count) {
	        ...
	        value: function addCodeItem(code, type, typeName, count) {
	            ...
	            (0, _jquery2.default)(this.cart_el).append(template); // 添加当前模板数据到文档中
	        ...
	        value: function getCount() {
	            ...
	            var count = this.computeCount(active, this.cur_play); // 返回注数
	            var range = this.computeBonus(active, this.cur_play); // 返回奖金范围
	
	            var money = count * 2; // 需要花费的金额
	            // 如果中奖
	            var win1 = range[0] - money; // 最小赢亏, 正是赢, 负是亏损
	            var win2 = range[1] - money; // 最大赢亏
	            var template = void 0;
	            ...
	            (0, _jquery2.default)('.sel_info').html(template);
	        }
	        ...
	        value: function getTotal() {
	            ...
	            (0, _jquery2.default)('.codelist li').each(function (index, item) {
	                count += (0, _jquery2.default)(item).attr('count') * 1; // 转为数字相加
	            });
	    ...
	    _createClass(Timer, [{
	        ...
	        value: function countdown(end, update, handle) {
	            ...
	            if (now - end > 0) {
	                ...
	                var last_time = end - now; // 截止时候 - 当前时间
	                var px_d = 1000 * 60 * 60 * 24; // 一天多少毫秒
	                var px_h = 1000 * 60 * 60; // 一小时多少毫秒
	                var px_m = 1000 * 60; // 一分多少毫秒
	                var px_s = 1000; // 一秒多少毫秒
	                var d = Math.floor(last_time / px_d); // 剩余时间还有多少天
	
	                var rest_h_ms = last_time - d * px_d; // 排除整天还剩下的毫秒
	                var h = Math.floor(rest_h_ms / px_h); // 剩余时间还有多少小时
	
	                var rest_m_ms = rest_h_ms - h * px_h; // 排除整天与小时还剩下的毫秒
	                var m = Math.floor(rest_m_ms / px_m); // 剩余时间还有多少分钟
	
	                var rest_s_ms = rest_m_ms - m * px_m; // 排除整天与小时与分钟还剩下的毫秒
	                var s = Math.floor(rest_s_ms / px_s); // 剩余时间还有多少秒
	...
	var Calculate = function () {
	    ...
	    _createClass(Calculate, [{
	        ...
	        value: function computeCount(active, play_name) {
	            ...
	            var arr = new Array(active).fill('0'); // 填充0为默认值
	        ...
	        value: function computeBonus(active, play_name) {
	            ...
	            var self = this;
	            var arr = new Array(play[1] * 1).fill(0); // 初始化玩法并填充初始值
	            var min = void 0,
	                max = void 0;
	            if (play[0] === 'r') {
	                var min_active = 5 - (11 - active); // 最小命中
	                if (min_active > 0) {
	                    // 如果最小命中大于0 至少是选7个数
	                    if (min_active - play[1] >= 0) {
	                        ...
	                        // 否则 玩法是任6,任7,任8 且 选中号码数大于或等于玩法的时候
	            ...
	            (function f(arr, size, result) {
	                ...
	                if (size > arrLen) {
	                    ...
	                    for (var i = 0; i < arrLen; i++) {
	                        var newResult = [].concat(result); // 保存上一次数组的结果
	        ...
	        value: function getOpenCode(issue) {
	            ...
	            return new Promise(function (resolve, reject) {
	                _jquery2.default.ajax({
	                    ...
	                    dataType: 'json',
	                    success: function success(res) {
	                        _this2.setOpenCode(res.data);
	                        resolve.call(_this2, res);
	                    },
	                    error: function error(err) {
	                        reject.call(err);
	                    }
